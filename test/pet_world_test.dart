...
void main() {
  test('loads message bubbles as platforms and reacts to emoji and GIF', () {
    ...
    expect(
        controller.objects
            .any((object) => object.kind == WorldObjectKind.platform),
        isTrue);

    final emoji = controller.objects
  ...
  test('inspecting GIF progresses through observation, hearts, and leap', () {
    ...
    // Tick into excited phase with hearts
    controller.tick(const Duration(milliseconds: 800));
    expect(controller.particles.any((p) => p.kind == ParticleKind.heart), isTrue);

    // Tick into approach and leap onto bottom edge of GIF bubble
    controller.tick(const Duration(milliseconds: 1200));
    expect(controller.position.dy, gifToy.bounds.bottom - 12);
  });
  ...
  test('landing on bubble triggers spring oscillation and dog bounces synchronously', () {
    ...
    expect(controller.position.dy, closeTo(expectedBaseY + initialDeflection, 0.001));
    ...
    expect(controller.position.dy, closeTo(expectedBaseY + settledDeflection, 0.001));
  });

  test('spawns alternating paw prints during walk/run and decays over time', () {
    final controller = PetWorldController();
    controller.loadRoom(FakeChatRepository.rooms.first);
  ...
  test('projects surface shadow accurately across ground, airborne jump, and bouncing platform', () {
    ...
    expect(controller.position.dy + 52.0, lessThan(controller.currentSurfaceY));
    expect(
    ...
    expect(controller.currentSurfaceY, closeTo(platform.bounds.top + deflection, 0.001));
    expect(controller.position.dy, closeTo(platform.bounds.top - 52.0 + deflection, 0.001));
  });

  test('switching pets updates selectedPet, petConfig, and spawns signature particles', () {
    final controller = PetWorldController();
    ...
    expect(controller.particles.any((p) => p.kind == ParticleKind.note), isTrue);
  });
}
